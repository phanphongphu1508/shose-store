...
function Login(props) {
  ...
  const linkgg =
    "https://accounts.google.com/o/oauth2/auth?scope=email&redirect_uri=http://localhost:8080/api/auth/login-google&response_type=code&client_id=721465338797-4lcqo4qql4vmevh90pnaahvo6tem4lsr.apps.googleusercontent.com&approval_prompt=force";
  const linkfb =
    "https://www.facebook.com/dialog/oauth?client_id=1101972766995519&redirect_uri=http://localhost:8080/api/auth/login-facebook";
  ...
  const loginAccount = () => {
    ...
    axios
      .post(hostname + "/api/auth/signin", bodyParameters)
      .then((Response) => {
  ...
  const getList = (e) => {
    ...
    // wishlist
    axios
      .get(hostname + `/api/wishlist`, config)
      .then((Response) => {
    ...
    //shopcart
    axios
      .get(hostname + `/api/shopcart`, config)
      .then((Response) => {
