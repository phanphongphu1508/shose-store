...
public class productsService {
    ...
    public productsDTO saveproducts(productsDTO productDTO,String username){
        ...
        // discount
         discountEntity discount = discountRepository.findById(productDTO.getDiscount()).get();
        if(discount != null){
    ...
    public ResultPage productPagination(int page, int size, String typeSort, Long categoryId,
        ...
        if (categoryId != null) { // sản phẩm theo danh mục
            Sort sort = orderBy.toUpperCase().equals("DESC") ?
                    Sort.by(Sort.Order.desc(typeSort)) :
                    Sort.by(Sort.Order.asc(typeSort));
            ...
            if (title.isPresent()) {
                ...
                if (isNumeric(title.get())) {
                    ...
                    Result = productsRepository.findByStatusAndCategory_idAndId(status, categoryId, id, pageable);
                } else {
                    Result = productsRepository.findByStatusAndCategory_idAndNameContaining(status, categoryId, title.get(), pageable);
                }
            } else {
    ...
    public List<productsDTO> getProductSale(int size) {
        ...
        // Lấy tất cả các discount còn hiệu lực
        List<discountEntity> listDiscount = discountRepository.findAll();

        // Nếu không có discount, trả về danh sách trống
        ...
        for (discountEntity discount : listDiscount) {
            // Kiểm tra nếu discount còn hiệu lực
            if (discount.getDeadline().after(new Date())) {
                // Truy vấn sản phẩm theo discount và trạng thái
                List<productsDTO> listTemp = parseList(productsRepository
                        .findByStatusAndDiscountEntitys(status, discount, pageable)
                        .getContent());

                // Thêm các sản phẩm vào danh sách và kiểm tra nếu đã đạt số lượng yêu cầu
